...
export class AverageRepositoryDatabase implements AverageRepository {
  async save(average: Average): Promise<void> {
    ...
    await connection.query(
      "delete from design_patterns.average where student_id = $1",
      [average.studentId]
    );
    await connection.query(
      "insert into design_patterns.average (student_id, value) values ($1, $2)",
  ...
  async getByStudentId(studentId: number): Promise<Average> {
    const connection = pgp()("postgres://postgres:123456@localhost:5432/app");
    const [averageData] = await connection.query(
      "select * from design_patterns.average where student_id = $1",
      [studentId]
    );
    await connection.$pool.end();
