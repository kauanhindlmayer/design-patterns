import BookRoom from "~/gof/structural/decorator/BookRoom";
import { BookingRepositoryDatabase } from "~/gof/structural/decorator/BookingRepository";
import CancelBooking from "~/gof/structural/decorator/CancelBooking";
import GetBookingByCode from "~/gof/structural/decorator/GetBookingByCode";
import ImportBooking from "~/gof/structural/decorator/ImportBooking";
import LogDecorator from "~/gof/structural/decorator/LogDecorator";
import { RoomRepositoryDatabase } from "~/gof/structural/decorator/RoomRepository";
import SecurityDecorator from "~/gof/structural/decorator/SecurityDecorator";
...
test("Deve importar uma lista de reservas", async function () {
  ...
  const importBooking = new SecurityDecorator(
    new LogDecorator(
      new ImportBooking(
        new LogDecorator(new BookRoom(roomRepository, bookingRepository))
      )
    )
  );
